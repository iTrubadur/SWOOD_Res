...
function Material(u_Name,u_Desc,u_Costfactor,u_LengthM,u_Width,u_Thickness,u_Costtype, u_Surf, u_Vol, u_Qte, u_GQte, u_Cost, u_GCost) {
	var pMaterial = {Name: u_Name,Desc : u_Desc , Costfactor: u_Costfactor, LengthM : parseFloat(u_LengthM), Width: parseFloat(u_Width), Thickness: parseFloat(u_Thickness) , Costtype : u_Costtype, Surf : parseFloat(u_Surf), Vol : parseFloat(u_Vol), Qte: parseFloat(u_Qte), GQte: parseFloat(u_GQte), Cost: parseFloat(u_Cost), GCost: parseFloat(u_GCost) };
	return pMaterial;
}

// Material_1: Chants
function Material_1(u_Name,u_Desc,u_Costfactor,u_LengthM,u_Width,u_Thickness,u_Costtype, u_Surf, u_Vol, u_Qte, u_GQte, u_Cost, u_GCost) {
	var pMaterial_1 = {Name: u_Name,Desc : u_Desc , Costfactor: u_Costfactor, LengthM : parseFloat(u_LengthM), Width: parseFloat(u_Width), Thickness: parseFloat(u_Thickness) , Costtype : u_Costtype, Surf : parseFloat(u_Surf), Vol : parseFloat(u_Vol), Qte: parseFloat(u_Qte), GQte: parseFloat(u_GQte), Cost: parseFloat(u_Cost), GCost: parseFloat(u_GCost) };
	return pMaterial_1;
}

// Material_2
function Material_2(u_Name,u_Desc,u_Costfactor,u_LengthM,u_Width,u_Thickness,u_Costtype, u_Surf, u_Vol, u_Qte, u_GQte, u_Cost, u_GCost) {
	var pMaterial = {Name: u_Name,Desc : u_Desc , Costfactor: u_Costfactor, LengthM : parseFloat(u_LengthM), Width: parseFloat(u_Width), Thickness: parseFloat(u_Thickness) , Costtype : u_Costtype, Surf : parseFloat(u_Surf), Vol : parseFloat(u_Vol), Qte: parseFloat(u_Qte), GQte: parseFloat(u_GQte), Cost: parseFloat(u_Cost), GCost: parseFloat(u_GCost)};
	return pMaterial_2;
}
...
//Si le type de cout est 1: Surfacique
	if (pMat.Costtype==1){
		document.write('<td><center>'+ Math.round(pMat.Surf*1000)/1000 + '<\center></td>');					
	}
//Sinon si le type de cout est 0: Volumique
	else if (pMat.Costtype==0)
	{
		document.write('<td><center>'+  Math.round(pMat.Vol*1000)/1000 + '</td>');		
	}
//Sinon il est linéaire
	else 
	{
		document.write('<td><center>'+ Math.round(pMat.LengthM)/1000 + '<\center></td>');
	}
//On ecrit le facteur de coût
	document.write('<td><center>'+ pMat.GQte + '<\center></td>');
//On écrit le brut (net*costfactor): 0=Volumique
	if (pMat.Costtype==0){
		document.write('<td><center>'+ Math.round(pMat.GQte*pMat.Vol*1000)/1000 + '<\center></td>');
		document.write('<td><center>m3<\center></td>');
	}
//On écrit le brut (net*costfactor): 1=Surfacique
	else if (pMat.Costtype==1){
		document.write('<td><center>'+ Math.round(pMat.GQte*pMat.Surf*1000)/1000 + '<\center></td>');
		document.write('<td><center>m2<\center></td>');
	}
//On écrit le brut (net*costfactor): Sinon linéaire
	else {
	document.write('<td><center>'+ Math.round(pMat.GQte*pMat.LengthM)/1000+ '<\center></td>');
...
function UpdateMaterial(u_Name, u_Desc, u_Costfactor, u_LengthM, u_Width, u_Thickness, u_Costtype, u_Qte, u_Gqte){
...
//Sinon on crée pMat et on initialise pMat.X=pX*u_Qte
	...
	{
		pMat=Material(u_Name,u_Desc,u_Costfactor,u_LengthM,u_Width,u_Thickness,u_Costtype, pSurf, pVol, u_Qte, u_Gqte);
...
function UpdateMaterial_1(u_Name, u_Desc, u_Costfactor, u_LengthM, u_Width, u_Thickness, u_Costtype, u_Qte, u_Gqte){
...
//Sinon on crée pMat_1 et on initialise pMat_1.X=pX*u_Qte
	...
	{
		pMat_1=Material_1(u_Name,u_Desc,u_Costfactor,u_LengthM,u_Width,u_Thickness,u_Costtype, pSurf, pVol, u_Qte, u_Gqte);
